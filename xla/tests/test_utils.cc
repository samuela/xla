        ...
        if (hlo_use.operand_number >= first_index) {
          ...
          if (!index_known_zeroes.has_value()) {
            if (get_index_known_zeroes != nullptr) {
              index_known_zeroes = get_index_known_zeroes(use, sliced_dim);
            }
          } else {
            std::optional<int64_t> current_known_zeroes = std::nullopt;
            if (get_index_known_zeroes != nullptr) {
              current_known_zeroes = get_index_known_zeroes(use, sliced_dim);
            }
            if (current_known_zeroes.has_value()) {
              // If we have multiple uses with different masks, we take the
              // union of known zeroes.
              index_known_zeroes =
                  index_known_zeroes.value() | current_known_zeroes.value();
            }
          }
        }
        break;
