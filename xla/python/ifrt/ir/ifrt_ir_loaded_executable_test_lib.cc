...
module {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      ArrayRef input,
      CreateArray({data.data()}, shape, shape, dtype, std::move(device_list0)));
...
module {
  ...
  ASSERT_OK_AND_ASSIGN(DeviceListRef second_device,
                       client_->MakeDeviceList({devices->devices()[1]}));
  ASSERT_OK_AND_ASSIGN(ArrayRef input, CreateArray({data.data()}, shape, shape,
                                                   dtype, first_device));
...
module {
  ...
  ASSERT_OK_AND_ASSIGN(ArrayRef input,
                       CreateArray({input_data.data()}, shape, shape,
...
module {
  ...
  ASSERT_OK_AND_ASSIGN(
      ArrayRef input,
      CreateArray({data.data()}, shape, shape, DType(DType::kS32), devices));
  ASSERT_OK_AND_ASSIGN(
      LoadedExecutable::ExecuteResult result,
      ifrt_ir_executable->Execute(absl::MakeSpan(&input, 1),
                                  ExecuteOptionsWithFillStatus(), devices));
