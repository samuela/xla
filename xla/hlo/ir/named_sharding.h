...
class NamedSharding {
 public:
  ...
  // Returns the partitions spec for the sharding which can be used to construct
  // a JAX PartitionSpec.
  //
  // This method is only used for JAX as it requires every dimension to be
  // closed and no sub-axis which is the case for boundary shardings.
  std::vector<std::vector<std::string>> JaxPartitionsSpec() const;
