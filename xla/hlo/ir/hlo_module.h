...
class HloModule {
 ...
 public:
  ...
  // fingerprint, or nullptr if not found. T must be a subclass of CacheEntry.
  // The object pointed to by the shared pointer remains valid as long as the
  // caller holds a reference to it, even if the entry is removed from the
  // cache. The entry remains in the cache until the module is destroyed or the
  // cache entry is overwritten.
  template <typename T>
